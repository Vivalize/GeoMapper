...
def getElevation(lat, lon):
	...
	if terrainPath == None:
		...
		print(latTop, latBottom, lonLeft, lonRight)
		print('latTop-latBottom=', str(latTop-latBottom), 'lat-latBottom=', lat-latBottom)
			
		pixWE = float(((lon-lonLeft)/(lonRight-lonLeft))*np.shape(terrainArray)[1])
		pixNS = float(((latTop-lat)/(latTop-latBottom))*np.shape(terrainArray)[0])
			
		print('At pixel', pixWE, pixNS)
			
		if pixWE < 0 or pixWE > np.shape(terrainArray)[1] or pixNS < 0 or pixNS > np.shape(terrainArray)[0]: return None
		else:
			print(np.shape(terrainArray))
			return float(terrainArray[int(pixNS)][int(pixWE)])
